...
public class GameBoard extends View{
    ...
    private boolean checkForCollision() {
        ...
        Rect r4 = new Rect(r1);
        if(r1.intersect(r2) && r2.intersect(r3)) {
            /*for (int i = r1.left; i<r1.right; i++) {
                for (int j = r1.top; j<r1.bottom; j++) {
                    if (bm1.getPixel(i-r4.left, j-r4.top)!=
                            Color.TRANSPARENT) {
                        if (bm2.getPixel(i-r2.left, j-r2.top) !=
                                Color.TRANSPARENT) {
                            if (bm3.getPixel(i-r3.left, j-r3.top) !=
                                    Color.TRANSPARENT) {
                                lastCollision = new Point(sprite3.x +
                                        i - r3.left, sprite3.y + j - r3.top);
                                return true;
                            }
                        }
                    }
